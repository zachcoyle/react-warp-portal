...
export const popup = BaseComponent =>
  withWarpSourceBoundingClientRect(
    createReactClass({
      propTypes: {
        strategy: PropTypes.oneOfType([PropTypes.string, PropTypes.func]),
        gap: PropTypes.number,
        bindingY: PropTypes.oneOf(["top", "bottom"]),
      ...
      render() {
        const getBottomPositionFromTop = (top, element) => {
          const childRect = ReactDOM.findDOMNode(
            element
        ...
        const {
          strategy,
          gap,
          warpSourceParentBoundingClientRect,
          ...props
        } = this.props;
        const yAxis =
          this.props.bindingY === "top" || !this.state.calculated
            ? { top: this.state.top }
